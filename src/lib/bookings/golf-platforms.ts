...
const HINTS: Record<GolfPlatform, string> = {
  chronogolf:
    'GOLF PLATFORM — CHRONOGOLF / LIGHTSPEED. ALWAYS use the "Visitors" tab (NEVER "Members" — Members forces a member login you do not have; if the page is on Members, click Visitors first). Work the accordion top-to-bottom: Date (click the day on the calendar; use the next-month arrow to reach the right month) → "18 holes" (the course is already chosen) → Players (set to the party size) → Tee time (a list of times appears — click the one at/nearest the requested time). ChronoGolf then shows a "Create an account" form even for visitors — this is NORMAL, not a dead end: the system auto-fills a strong password in the Password + Repeat fields, so just tick any required box and click "Sign up" to create the account on the customer\'s behalf and continue to the booking. Then stop at the card step. Do NOT stop at the signup screen and do NOT click "login".',
  foreup:
