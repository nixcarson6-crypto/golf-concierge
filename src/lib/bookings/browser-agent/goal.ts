...
## Private / members-only golf — BAIL FAST with a clear message (don't sit there)
Many upscale courses are PRIVATE clubs — you cannot book a public tee time, full stop. Recognise this EARLY and stop; do not keep hunting for a form that doesn't exist (a real run sat 2.5 minutes on a private club doing nothing). Tell-tale signs of a private / members-only club:
- The site's main nav has **"Member Login" / "Membership"** and the golf is shown as a **showcase** ("Experience our finest courses", course photos/descriptions) with **NO public "Book a Tee Time" / availability / date-picker** anywhere.
- Text like "members only", "members and their guests", "private club", "registered resort guests only", "must be a member", "must be a registered guest", or only a phone number / "call the pro shop" / "request more information".
- A login wall with NO guest/visitor option (and it's NOT a ChronoGolf-style "Visitors" tab you can use).
You do NOT have a membership or a resort-guest reservation at that specific club, so you CANNOT complete it. The MOMENT these signs are clear (you do not need to click everything first), STOP and report \`needs_review\` with reason "members_only" and a message the customer will see, e.g.: **"This course is a private members-only club, so we can't book a public tee time here — our concierge can arrange access or suggest a nearby course you can play."** Include the pro-shop phone number if shown. Bailing in under ~2 minutes is the RIGHT outcome here — it's not a failure, it lets us route the customer to a course they can actually play.
