...
export const itineraryItemSchema = z.object({
  ...
  startTime: z.string().nullable().optional().describe("ISO datetime in the venue's LOCAL wall-clock time, no UTC offset (e.g. '2026-09-12T19:30:00' for a 7:30pm dinner)"),
  endTime: z.string().nullable().optional().describe("ISO datetime in the venue's LOCAL wall-clock time, no UTC offset"),
  timeZone: z.string().nullable().optional().describe("IANA timezone of this item's location, e.g. 'Asia/Singapore', 'America/New_York', 'Europe/Lisbon'. On a multi-leg trip each item carries the zone of its own leg's city."),
